...
- [ ] Persistent chapter menu in the nav bar is present on every screen, highlights the **current chapter**, shows each chapter's state (done/available/locked), and jumps to any unlocked chapter.
- [ ] Forward/back and module jumps work; locked sequence (if any) enforced.
- [ ] **Forward is gated:** advancing is blocked until the current block/chapter's required interactions are complete.
...
- [ ] **Buttons are clean of text clutter** — no chapter-name labels, no "finish the interactions" note, no "required" badge on interactions. (Visual state IS wanted — see next line.)
- [ ] **Locked forward control shows clear visual state** — dimmed/greyed/locked look while required interactions are incomplete, and visibly changes to active/colored once they're done; arrows/animation/shimmer are fine.
- [ ] Prev/Next footer buttons are arrows only (← →), with a descriptive aria-label for screen readers — not the chapter title.
- [ ] A blocked advance surfaces a clear floating message ("complete the interactions in this chapter") — never a silent dead button; message is announced for screen readers.
...
- [ ] Wrong answers do NOT satisfy a gate — only correct completion (or revealed-and-acknowledged answer) unlocks forward progress.
- [ ] Every assessed interaction (drag&drop, scenario, quiz, matching, sequencing, hotspot-ID) has: a smart "try again" that returns only the wrong items and keeps correct ones, explanatory feedback naming what's wrong, and an answer reveal after 3 tries — the learner is never stuck.
...
- [ ] Tested at 360–390px (mobile), tablet, and desktop widths — real rendering, not just a resize assumption.
- [ ] On mobile: buttons sized to content (not oversized/full-width hogging the screen); sticky header/footer compact; no element covers content; no horizontal scroll or clipping; text readable without zoom; tap targets reachable.
