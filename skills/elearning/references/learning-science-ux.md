...
- **Mobile-first (קריטי — בדקי בנייד ממש):** design for 360px width first, then scale up. The lomda is very often consumed on a phone, so a "looks fine on desktop" build is not done. Concrete rules:
  - Touch targets ≥ 44px, but **buttons must not dominate the screen** — full-width or oversized buttons eat scarce real estate. Size them to their content with comfortable padding; reserve full-width only for a single primary action, and even then keep height modest.
  - The footer/nav bar and any sticky chrome should be **compact on mobile** — a tall sticky header/footer plus a toast can swallow half a phone screen. Prefer icon/arrow controls over text labels in the bars (this is why prev/next are arrows). Consider shrinking or auto-hiding chrome on small viewports.
  - Content gets the screen, not decoration. Reduce padding, font sizes, and margins at small breakpoints; stack multi-column layouts to one column; let interactions (cards, drag&drop) reflow vertically.
  - Test the real thing at 360–390px: nothing clipped, no horizontal scroll, no element covering content, tap targets reachable with a thumb, text readable without zoom.
- **Touch targets ≥ 44px** without making controls visually bulky — padding and hit-area, not giant boxes.
