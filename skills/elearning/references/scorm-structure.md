...
- **Mobile responsive (mandatory, test for real):** flex/grid, % and rem, test at 360–390px width. Buttons sized to content (not oversized/full-width by default); compact sticky chrome (header/footer) so it doesn't swallow the screen; arrows not text labels in the nav bars; content prioritized over decoration; single-column stacking at small breakpoints; no clipping or horizontal scroll. See the mobile-first rules in `learning-science-ux.md`.
...
- **Persistent chapter menu in the nav bar (תפריט פרקים בסרגל הניווט)** — a menu in the top bar, available from every screen, that lists all chapters and **shows which chapter the learner is currently in** (clearly highlighted). Each entry shows its state (done ✓ / available ○ / locked 🔒) and lets the learner jump to any unlocked chapter. This is separate from the full-page nav map — it's the always-present wayfinding control. It must respect the same gating (locked chapters are shown but not openable) and be keyboard-accessible (open/close, Esc to close, focusable items).
